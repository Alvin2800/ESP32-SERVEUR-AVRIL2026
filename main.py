...
db_config={"host": os.getenv("MYSQLHOST"),
...
def data():
    try:
        ...
        inser_data(str(timestamp),distance,alert)
        print("données recues",{"distance":distance,
                            "alert":alert
           })
        return "ok",200
    except Exception as e:
...
def logs():
    try:
        ...
        cursor=conn.cursor()
        cursor.execute("""
        SELECT  (timestamp,distance,alert) FROM measuring
